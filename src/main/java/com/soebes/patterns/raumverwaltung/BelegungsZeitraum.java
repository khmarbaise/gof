...
public class BelegungsZeitraum implements Comparable<BelegungsZeitraum> {
    ...
    public BelegungsZeitraum(Date anfangBelegung, Date endeBelegung) throws BelegungsZeitraumEndeVorAnfangException,
            BelegungsZeitraumAnfangGleichEndeException {
        if (endeBelegung.before(anfangBelegung)) {
            throw new BelegungsZeitraumEndeVorAnfangException("Das Ende der Belegung liegt vor dem Anfang der Belegung");
        }
        if (anfangBelegung.equals(endeBelegung)) {
            throw new BelegungsZeitraumAnfangGleichEndeException("Anfang und Ende sind gleich!");
        }
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + ((anfangBelegung == null) ? 0 : anfangBelegung.hashCode());
        result = prime * result + ((endeBelegung == null) ? 0 : endeBelegung.hashCode());
        return result;
    }
    ...
    public boolean contains(BelegungsZeitraum zeitraum) {
        ...
        if (this.getAnfangBelegung().after(zeitraum.getAnfangBelegung())
                || this.getAnfangBelegung().equals(zeitraum.getAnfangBelegung())) {
            if (this.getEndeBelegung().before(zeitraum.getEndeBelegung())
                    || this.getEndeBelegung().equals(zeitraum.getEndeBelegung())) {
                // Liegt drin
                result = true;
            }
        }
